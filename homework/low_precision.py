...
class Linear4Bit(torch.nn.Module):
    ...
    def _load_state_dict_pre_hook(
        ...
        # Check if the weight key exists in the state_dict
        if f"{prefix}weight" in state_dict:
            weight = state_dict[f"{prefix}weight"]
            del state_dict[f"{prefix}weight"]

            # Quantize the weights
            weight_q4, weight_norm = block_quantize_4bit(weight.view(-1), group_size=self._group_size)
            self.weight_q4.copy_(weight_q4.view(self.weight_q4.shape))
            self.weight_norm.copy_(weight_norm.view(self.weight_norm.shape))

            # Remove unexpected keys related to bias
...
class BigNet4Bit(torch.nn.Module):
    """
    A BigNet where all weights are in 4-bit precision. Use the Linear4Bit module for this.
